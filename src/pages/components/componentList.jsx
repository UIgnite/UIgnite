...
const ComponentCard = ({name="Button", imageUrl="https://placehold.co/600x400", desc="Button is a button whose role is button which is button is called button."}) => {
  ...
  return (
    <div onClick={()=>navigate("/components/button")} role="button" tabIndex={0} className="cursor-pointer group hover:bg-black/60 rounded-lg p-3 transition-colors">
      <div className="mb-2">
        <img className="group-hover:scale-105 transition-all rounded-md" src={imageUrl} alt={name}/> 
      </div>
