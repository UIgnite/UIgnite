...
function getCodeStrById(compId: string | undefined) {
  ...
  return codeString.find(
    (codeStr) => codeStr.id!==undefined &&  codeStr.id.trim().toLowerCase() === compId.trim().toLowerCase()
  );
}

function getPreviousComponentbyId(compId: string | undefined) {
  if (!compId) return null;
  const currInd: number = components.findIndex((comp) => comp.id!==undefined &&  comp.id === compId);
  if (currInd === 0) {
